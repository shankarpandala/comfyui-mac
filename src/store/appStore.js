...
const useAppStore = create(
  persist(
    (set, get) => ({
      ...
      unmarkSectionComplete: (subjectId, chapterId, sectionId) => {
        const key = `${subjectId}::${chapterId}::${sectionId}`
        set((state) => ({
          completedSections: state.completedSections.filter((k) => k !== key),
        }))
      },
      ...
      getSubjectProgress: (subjectId, totalSections) => {
        ...
        const completed = get().completedSections.filter((k) => k.startsWith(`${subjectId}::`)).length
        return Math.round((completed / totalSections) * 100)
      },
    }),
    {
      name: 'comfyui-mac-app-store',
      partialize: (state) => ({
        theme: state.theme,
